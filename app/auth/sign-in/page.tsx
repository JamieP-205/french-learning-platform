...
export default function SignInPage() {
  ...
  const accountSyncReady = isAccountSyncReady({
    NEXT_PUBLIC_ACCOUNT_SYNC_READY: process.env.NEXT_PUBLIC_ACCOUNT_SYNC_READY,
  ...
  return (
    <main className="page-shell flex min-h-screen items-center justify-center py-10">
      <section className="card w-full max-w-md">
        ...
        <h1 className="mt-2 text-3xl font-black">{isSignUp ? "Create your account" : "Welcome back"}</h1>
        <p className="mt-3 text-sm text-ink/65">
          {isSignUp
            ? "Account sync is optional. You can already learn publicly with browser-only progress."
            : "Sign in if you already have account sync; otherwise start learning without an account."}
        </p>
        ...
        {accountSyncReady ? (
          ...
          <div className="mt-6 space-y-4">
            <p className="status-error">
              Account sync is not open yet. Confirmation-email delivery must be verified end to end before sign-up and sign-in are enabled.
            </p>
